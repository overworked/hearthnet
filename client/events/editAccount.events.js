Template.editAccount.events({
    'submit #account-edit-form': function (e, templateInstance) {
        ...
        var preferredDecksToFace = templateInstance.find('#preferredDecksToFace').value.split(',').join(' ').split(' ').filter(function (e) {
            return e;
        });
        var dislikedDecksToFace = templateInstance.find('#dislikedDecksToFace').value.split(',').join(' ').split(' ').filter(function (e) {
            return e;
        });
        var decksUsed = templateInstance.find('#decksUsed').value.split(',').join(' ').split(' ').filter(function (e) {
            return e;
        });
